...
function Dashboard({ role }) {
    ...
    useEffect(() => {
        Promise.all([
            ...
            .catch(error =>
                setErreur(getErrorMessage(error, "Impossible de charger le tableau de bord."))
            );
    }, []);
    ...
    const stats = useMemo(() => {
        ...
        ventesFiltrees.forEach(vente => {
            const caissier = vente.caissierNom || "Non précisé";
            ...
            (vente.lignes || []).forEach(ligne => {
                ...
                const categorie = article?.categorieNom || "Sans catégorie";
        ...
        return {
            ...
            totalAchats,
            caisseJour: ventes
                .filter(v => v.dateVente === aujourdhui() && v.modePaiement !== "CREDIT")
                .reduce((s, v) => s + Number(v.total || 0), 0),
            gainEstime,
            stockDisponible,
            stockFaible,
    ...
    }, [achatsFiltres, articles, articleParId, clients, ventes, ventesFiltrees]);
    ...
    return (
        <div className="container mt-4">
            ...
            <div className="d-flex justify-content-between align-items-center mb-3">
                <div>
                    <h2>{estAdmin ? "Tableau de bord administrateur" : "Tableau de bord caissier"}</h2>
                    <p className="text-muted mb-0">
                        {estAdmin
                            ? "Statistiques complètes : caisse, stock, crédit, gains et recettes."
                            : "Vue limitée : caisse du jour, ventes et crédits client."}
                    </p>
                </div>
                <div className="badge bg-dark fs-6">{estAdmin ? "ADMIN" : "CAISSIER"}</div>
            </div>
            ...
            <div className="row g-3">
                <Carte couleur="success" titre="Caisse du jour" valeur={`${montant(stats.caisseJour)} DT`} />
                <Carte couleur="primary" titre="Recettes période" valeur={`${montant(stats.totalVentes)} DT`} />
                <Carte couleur="success" titre="Ventes espèces" valeur={`${montant(stats.totalEspece)} DT`} />
                <Carte couleur="danger" titre="Ventes crédit" valeur={`${montant(stats.totalCredit)} DT`} />

                {estAdmin && (
                    <>
                        <Carte couleur="info" titre="Achats période" valeur={`${montant(stats.totalAchats)} DT`} />
                        <Carte couleur="dark" titre="Gain estimé" valeur={`${montant(stats.gainEstime)} DT`} />
                        ...
                        <Carte couleur="danger" titre="Crédit clients total" valeur={`${montant(stats.creditClients)} DT`} />
                        <Carte couleur="secondary" titre="Ventes annulées" valeur={stats.ventesAnnulees} />
                    </>
                )}
            </div>
                ...
                <div className="row mt-4">
                    <div className="col-md-12 mb-4">
                        ...
                        <table className="table table-bordered bg-white">
                            ...
                            <tr><th>Caissier</th><th>Nombre ventes</th><th>Recette</th><th>Crédit</th></tr>
                            </thead>
                            <tbody>
                    ...
                    <div className="col-md-6">
                        ...
                        <table className="table table-bordered bg-white">
                            <thead className="table-dark">
                            <tr><th>Article</th><th>Qté</th><th>Recette</th><th>Gain estimé</th></tr>
                            </thead>
                            <tbody>
                    ...
                    <div className="col-md-6">
                        <TableauTitre titre="Gain / recette par catégorie" />
                        <table className="table table-bordered bg-white">
                            <thead className="table-dark">
                            <tr><th>Catégorie</th><th>Recette</th><th>Gain estimé</th></tr>
                            </thead>
                            <tbody>
